...
// @version      1.2
// @description  patao
// @author       pato // edited and arranged by MARCOS # 4655 // Porrete<3
// @match        http://pixelzone.io/*
// @match        https://pixelzone.io/*
// @homepage
// @updateURL    https://raw.githubusercontent.com/Poraett/pato/master/minimap.user.js
// @downloadURL  https://raw.githubusercontent.com/Poraett/pato/master/minimap.user.js
// @grant        none
// ==/UserScript==
...
var range = 25;
window.baseTepmlateUrl = 'https://raw.githubusercontent.com/Poraett/pato/master/';
...
vers = "3n1x";
    ...
    div.innerHTML = '<style>.grecaptcha-badge{display: none;}</style>   <div id="minimapbg" style="position: absolute; right: 5em; bottom: 0em;">' +
'<div class="posy" id="posyt" style="background-size: 100%; background-image: url(https://i.imgur.com/mUyOrU5.png); color: rgb(255, 255, 255); text-align: center; line-height: 42px; vertical-align: middle; width: auto; height: auto; border-radius: 10px; padding: 5px;">' +
        '<div id="minimap-text" style="display: none;"></div>' +
        '<div id="minimap-box" style="position: relative;width:310px;height:250px">' +
        '<canvas id="minimap" style="width: 100%; height: 100%;z-index:3;position:absolute;top:0;left:0;"></canvas>' +
        '<canvas id="minimap-board" style="width: 100%; height: 100%;z-index:4;position:absolute;top:0;left:0;"></canvas>' +
        '<canvas id="minimap-cursor" style="width: 100%; height: 100%;z-index:5;position:absolute;top:0;left:0;"></canvas>' +
        '</div><div id="minimap-config" style="line-height:20px;">' +
        '<span id="hide-map" style="cursor:pointer;">Hide  ' +
        '</span><span id="follow-mouse" style="cursor:pointer;"Follow Cursor' +
        '</span>  |  Zoom: <span id="zoom-plus" style="cursor:pointer;font-weight:monospace;">+</span>  /  ' +
        '<span id="zoom-minus" style="cursor:pointer;font-weight:monospace;">-</span>' +
        '</span>  |  DC: <span id="discord" style="cursor:pointer;">  PoraetΣ#0167' +
        '</div>' +
        '</div>';
...
function loadTemplates() {
    ...
    if (needed_templates.length == 0) {
        if (zooming_in == false && zooming_out == false) {
            document.getElementById("minimap-box").style.display = "none";
            document.getElementById("minimap-text").style.display = "block";
            document.getElementById("minimap-text").innerHTML = "None templates here.";
        }
    } else {
        document.getElementById("minimap-box").style.display = "block";
        document.getElementById("minimap-text").style.display = "none";
